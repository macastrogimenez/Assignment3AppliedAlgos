#!/usr/bin/env python3
"""
Script to create a log-log scatter plot comparing Dijkstra and Contraction Hierarchies.
...
# Read the CSV file
df = pd.read_csv('experiment_resultsCH.csv')

# Separate data by algorithm
dijkstra = df[df['algorithm'] == 'D']
ch = df[df['algorithm'] == 'CH']

# Create the plot
plt.figure(figsize=(12, 8))

# Plot Dijkstra (blue) and Contraction Hierarchies (green)
plt.scatter(dijkstra['time_ns'], dijkstra['relaxed_edges'], 
           c='blue', alpha=0.6, s=30, label='Dijkstra', edgecolors='none')
plt.scatter(ch['time_ns'], ch['relaxed_edges'], 
           c='green', alpha=0.6, s=30, label='Contraction Hierarchies', edgecolors='none')
...
plt.title('Algorithm Performance: Dijkstra vs Contraction Hierarchies\n(Log-Log Scale)', fontsize=14, fontweight='bold')
...
ch_avg_time = ch['time_ns'].mean()
ch_avg_relaxed = ch['relaxed_edges'].mean()

speedup = d_avg_time / ch_avg_time if ch_avg_time > 0 else 0
...
stats_text = f'Average Performance:\n' \
             ...
             f'CH: {ch_avg_relaxed:.0f} edges, {ch_avg_time/1e6:.2f} ms\n' \
...
# Save the plot
plt.savefig('ch_vs_dijkstra_loglog.png', dpi=300, bbox_inches='tight')
print("Plot saved as 'ch_vs_dijkstra_loglog.png'")
...
print(f"  Average relaxed edges: {ch_avg_relaxed:.2f}")
print(f"  Average time: {ch_avg_time:.2f} ns ({ch_avg_time/1e6:.4f} ms)")
