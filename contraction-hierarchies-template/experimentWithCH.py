...
def run_jar(graph_file: str, algorithm: str, vertex1: int, vertex2: int) -> ExperimentResult:
    ...
    """
    # Use preprocessed graph for CH algorithm
    if algorithm == "CH":
        graph_file = CH_GRAPH_FILE
...
def main():
    ...
    # Check if CH preprocessed graph exists
    ch_graph_path = Path(CH_GRAPH_FILE)
    if not ch_graph_path.exists():
        print(f"Error: Preprocessed CH graph file not found at {CH_GRAPH_FILE}")
        print("Please preprocess the graph first for CH algorithm.")
        return
